...
@user_admin
@loggable
def fullpromotes(update: Update, context: CallbackContext) -> str:
    bot = context.bot
    args = context.args
    ...
    bot.sendMessage(
        chat.id,
        f"Sucessfully promoted With Full Rights <b>{user_member.user.first_name or user_id}</b>!",
        parse_mode=ParseMode.HTML)
...
FULLPROMOTE_HANDLER = DisableAbleCommandHandler("fullpromotes", fullpromotes)
